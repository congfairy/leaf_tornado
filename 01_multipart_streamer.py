...
MAX_STREAMED_SIZE = 0.6 * TB  # Max. size streamed in one request!
...
class MyStreamer(MultiPartStreamer):
    ...
    def create_part(self, headers):
        ...
        temp = TemporaryFileStreamedPart(self, headers, tmp_dir=os.path.dirname(TMP_DIR))
	#temp = TemporaryFileStreamedPart(self, headers, tmp_dir=TMP_DIR)
        #temp.move("/root")
        return temp
...
Note: firefox has a bug that may cause problems when uploading large files.
See https://bugzilla.mozilla.org/show_bug.cgi?id=1227963
